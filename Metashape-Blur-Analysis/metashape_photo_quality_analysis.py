...
def init():
    ...
    print(root_folder)
    photos = findFiles(root_folder, [".jpg", ".jpeg", ".tif", ".tiff"])
    if len(photos) == 0:
...
def timeKeeping():
    ...
    # timedelta_string = str(timedelta(seconds = duration))
    # x = timedelta_string.split(':')
    # print(f"Blur analysis completed in {x[0]} Hours, {x[1]} Minutes, {x[2]} Seconds")
